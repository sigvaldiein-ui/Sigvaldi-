...
async def chat_stream_endpoint(query: str = "Hæ"):
    """
    Sprint 91/92 Core — Server-Sent Events Foundation with UI Safety Brake.
    Includes Token Budget Analytics and Scope Protection verified by Aðal.
    """
    import time, asyncio
    ...
    async def event_generator():
        ...
        try:
            ...
            stream_latency_ms = (time.time() - start_stream) * 1000
            mock_think_len = 120 
            token_efficiency_ratio = tokens_sent / (mock_think_len if mock_think_len > 0 else 1)
            
            _audit_log(
                user_id="stream_user",
                query=query,
                result={
                    "response": "".join(mock_response_chunks),
                    "metadata": {
                        "actions_logged": f"Stream verified. Efficiency: {token_efficiency_ratio:.2f}",
                        "tier": "streaming_v2"
                    }
                },
                context={"search_text": "Streaming active check", "file_context": ""},
