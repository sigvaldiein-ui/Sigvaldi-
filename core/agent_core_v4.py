...
def ask_llm(system_prompt: str, user_prompt: str, temp: float = 0.1) -> str:
    ...
    payload = {
        ...
        "temperature": temp,
        "max_tokens": 4096,  # prevent runaway cost per call
    }
    try:
        # S5: 110s timeout — leaves 10s buffer before web_server 120s limit
        r = requests.post(url, headers=headers, json=payload, timeout=110)
        r.raise_for_status()
        return r.json()["choices"][0]["message"]["content"].strip()
    except requests.exceptions.Timeout:
        return "VILLA: Greining tók of langan tíma. Reyndu aftur með styttri fyrirspurn."
